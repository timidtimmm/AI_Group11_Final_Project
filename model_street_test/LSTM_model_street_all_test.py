...
class LSTMModel(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers=1, dropout=0.0):
        ...
        self.fc = nn.Sequential(                            #add more dropout layers and fully connected layers
...
def main():
    ...
    for name, group in grouped:                     #train group by group
        ...
        model = LSTMModel(input_size=len(features), hidden_size=64, num_layers=4, dropout=0.1)
        ...
        #increase the number of epoch to 30
        ...
        os.makedirs("./model_street_test/models_street_all_test", exist_ok=True)
        torch.save(model.state_dict(), f"./model_street_test/models_street_all_test/LSTM_model_{street_name}.pth")
        joblib.dump(scaler, f"./model_street_test/models_street_all_test/scaler_x_{street_name}.pkl")
        joblib.dump(target_scaler, f"./model_street_test/models_street_all_test/scaler_y_{street_name}.pkl")

    # Save results to CSV
    results_df = pd.DataFrame(results)
    results_df.to_csv("./model_street_test/LSTM_street_results_all_test.csv", index=False)
    print("\nResults saved to LSTM_street_results_all_test.csv")
    
    # Save all predictions to CSV
    all_preds_df = pd.DataFrame(all_preds)
    all_preds_df.to_csv("./model_street_test/LSTM_street_predict_all_test.csv", index=False)
    print("All true and predicted values saved to LSTM_street_predict_all_test.csv")
