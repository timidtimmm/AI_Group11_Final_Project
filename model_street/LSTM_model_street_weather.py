...
def main():
    ...
    for name, group in grouped:                     #train group by group
        ...
        os.makedirs("models_street_weather", exist_ok=True)
        torch.save(model.state_dict(), f"./models_street_weather/LSTM_model_{street}.pth")


    # Save results to CSV
    results_df = pd.DataFrame(results)
    results_df.to_csv("LSTM_results_weather.csv", index=False)
    print("\nResults saved to LSTM_results_weather.csv")
