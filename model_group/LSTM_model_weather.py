...
def main():
    ...
    for name, group in grouped:                     #train group by group
        ...
        os.makedirs("models_group_weather", exist_ok=True)
        torch.save(model.state_dict(), f"./models_group_weather/LSTM_model_{boro}_{street}.pth")

    # Save results to CSV
    results_df = pd.DataFrame(results)
    results_df.to_csv("LSTM_group_results_weather.csv", index=False)
