...
#training the model by grouping with (boro, street) and only use the demand data
def main():
    ...
    for name, group in grouped:                     #train group by group
        ...
        boro_name = encoders['Boro'].inverse_transform([int(boro_code)])[0]          #original borough name
        street_name = encoders['street'].inverse_transform([int(street_code)])[0]    #original street name
        print(f"\n=== Training for group: {name} ===")
        print(f" Boro: {boro_name} Street: {street_name} ")
        ...
        os.makedirs("./model_group/models_group_demand", exist_ok=True)
        torch.save(model.state_dict(), f"./model_group/models_group_demand/LSTM_model_{boro_name}_{street_name}.pth")
        joblib.dump(scaler, f"./model_group/models_group_demand/scaler_x_{boro_name}_{street_name}.pkl")
        joblib.dump(target_scaler, f"./model_group/models_group_demand/scaler_y_{boro_name}_{street_name}.pkl")

    # Save results to CSV
    results_df = pd.DataFrame(results)
    results_df.to_csv("./model_group/LSTM_group_results_demand.csv", index=False)
    print("\nResults saved to LSTM_group_results_demand.csv")
            
    # Save all predictions to CSV
    all_preds_df = pd.DataFrame(all_preds)
    all_preds_df.to_csv("./model_group/LSTM_group_predict_demand.csv", index=False)
    print("All true and predicted values saved to LSTM_group_predict_demand.csv")
